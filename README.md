...
的格子，再點擊目標格子即可完成走棋；當兵走到底線可升變時，會彈出視窗讓你
點選要升變成皇后、城堡、主教或騎士（點選視窗外可取消這次選擇）。棋盤左右
兩側是黑色留白區：左上角顯示黑方西洋棋鐘、右下角顯示白方西洋棋鐘（各 5
分鐘倒數，輪到誰走棋該方時間就會高亮，剩餘 30 秒內轉紅），右上角則列出
快捷鍵提示。若某一方時間歸零會立即判負。按 `R` 鍵可重新開始一局，按
`Ctrl+Z` 可悔棋，按 `Esc` 鍵可回到主選單。
