...
- 悔棋（undo）：GUI 按 `Ctrl+Z`、CLI 輸入 `undo`，可連續悔多步，
  即使已將死/和局/超時也能悔回上一步
- 西洋棋鐘計時器（GUI）：雙方各 5 分鐘，對局畫面頂端即時顯示倒數，
  剩餘 30 秒內轉紅色警示，持棋時間歸零自動判負
...
點選要升變成皇后、城堡、主教或騎士（點選視窗外可取消這次選擇）。畫面頂端
會顯示雙方西洋棋鐘（各 5 分鐘倒數，輪到誰走棋該方時間就會高亮），若某一方
時間歸零會立即判負。按 `R` 鍵可重新開始一局，按 `Ctrl+Z` 可悔棋，按 `Esc`
鍵可回到主選單。
