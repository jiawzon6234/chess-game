...
再點擊目標格子即可完成走棋；當兵走到底線可升變時，會彈出視窗讓你點選要
升變成皇后、城堡、主教或騎士（點選視窗外可取消這次選擇）。棋盤左右兩側是
黑色留白區：左上角顯示黑方西洋棋鐘、右下角顯示白方西洋棋鐘（輪到誰走棋該
方時間就會高亮，剩餘 30 秒內轉紅，選擇無限制時顯示 ∞），右上角則列出快捷
鍵提示（若該局未開啟悔棋，不會顯示 Ctrl+Z 提示）。若某一方時間歸零會立即
判負。按 `R` 鍵可用同樣設定重新開始一局，若該局開啟了悔棋可按 `Ctrl+Z`
悔棋，按 `Esc` 鍵可回到主選單。
